...
test("every catalog entry points to a real artifact with the expected size",()=>{for(const release of releases){const stat=fs.statSync(artifactPath(release.filename));assert.equal(stat.size,release.size,release.filename)}});
